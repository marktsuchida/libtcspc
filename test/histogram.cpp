...
TEMPLATE_TEST_CASE("Histogram, zero bins", "[histogram]", saturate_on_overflow,
    ...
    auto out = capture_output<event_set<
        histogram_event<u16>, accumulated_histogram_event<u16>, misc_event>>();
    auto in = feed_input<
    ...
    in.feed(reset_event{});
    REQUIRE(out.check(
        accumulated_histogram_event<u16>{0, 0, {}, 0, 0, false, false}));
    in.feed_end();
    REQUIRE(out.check(
        accumulated_histogram_event<u16>{0, 0, {}, 0, 0, false, true}));
...
TEMPLATE_TEST_CASE("Histogram, no overflow", "[histogram]",
    ...
    auto out = capture_output<
        event_set<histogram_event<u16>, accumulated_histogram_event<u16>>>();
    ...
    REQUIRE(out.check(
        accumulated_histogram_event<u16>{42, 43, {1, 1}, 2, 0, true, false}));
    in.feed(bin_increment_event<u32>{45, 0});
    REQUIRE(out.check(histogram_event<u16>{45, 45, {1, 0}, 1, 0}));
    in.feed_end();
    REQUIRE(out.check(
        accumulated_histogram_event<u16>{45, 45, {1, 0}, 1, 0, true, true}));
    REQUIRE(out.check_end());
}
...
TEST_CASE("Histogram, saturate on overflow", "[histogram]") {
    ...
    SECTION("Max per bin = 0") {
        ...
        in.feed_end();
        REQUIRE(out.check(
            accumulated_histogram_event<u16>{42, 42, {0}, 1, 1, true, true}));
        REQUIRE(out.check_end());
    }
    ...
    SECTION("Max per bin = 1") {
        ...
        in.feed(reset_event{44});
        REQUIRE(out.check(
            accumulated_histogram_event<u16>{42, 43, {1}, 2, 1, true, false}));
        in.feed(bin_increment_event<u32>{45, 0});
        REQUIRE(out.check(histogram_event<u16>{45, 45, {1}, 1, 0}));
        in.feed_end();
        REQUIRE(out.check(
            accumulated_histogram_event<u16>{45, 45, {1}, 1, 0, true, true}));
        REQUIRE(out.check_end());
    }
}
...
TEST_CASE("Histogram, reset on overflow", "[histogram]") {
    ...
    SECTION("Max per bin = 1") {
        ...
        in.feed(bin_increment_event<u32>{43, 0}); // Overflow
        REQUIRE(out.check(
            accumulated_histogram_event<u16>{42, 42, {1}, 1, 0, true, false}));
        REQUIRE(out.check(histogram_event<u16>{43, 43, {1}, 1, 0}));
        in.feed_end();
        REQUIRE(out.check(
            accumulated_histogram_event<u16>{43, 43, {1}, 1, 0, true, true}));
        REQUIRE(out.check_end());
    }
}
...
TEST_CASE("Histogram, stop on overflow", "[histogram]") {
    ...
    SECTION("Max per bin = 0") {
        ...
        in.feed(bin_increment_event<u32>{42, 0}); // Overflow
        REQUIRE(out.check(
            accumulated_histogram_event<u16>{0, 0, {0}, 0, 0, false, true}));
        REQUIRE(out.check_end());
    }
    ...
    SECTION("Max per bin = 1") {
        ...
        in.feed(bin_increment_event<u32>{43, 0}); // Overflow
        REQUIRE(out.check(
            accumulated_histogram_event<u16>{42, 42, {1}, 1, 0, true, true}));
        REQUIRE(out.check_end());
    }
}
...
TEMPLATE_TEST_CASE("Accumulate histograms, zero bins",
    ...
    in.feed(reset_event{});
    REQUIRE(out.check(
        accumulated_histogram_event<u16>{42, 43, {}, 0, 0, true, false}));
    in.feed(bin_increment_batch_event<u32>{42, 43, {}});
    REQUIRE(out.check(histogram_event<u16>{42, 43, {}, 0, 0}));
    in.feed_end();
    REQUIRE(out.check(
        accumulated_histogram_event<u16>{42, 43, {}, 0, 0, true, true}));
    REQUIRE(out.check_end());
}
...
TEMPLATE_TEST_CASE("Accumulate histograms, no overflow",
    ...
    REQUIRE(out.check(
        accumulated_histogram_event<u16>{42, 45, {2, 1}, 3, 0, true, false}));
    in.feed(bin_increment_batch_event<u32>{47, 48, {1}});
    REQUIRE(out.check(histogram_event<u16>{47, 48, {0, 1}, 1, 0}));
    in.feed_end();
    REQUIRE(out.check(
        accumulated_histogram_event<u16>{47, 48, {0, 1}, 1, 0, true, true}));
    REQUIRE(out.check_end());
}
...
TEST_CASE("Accumulate histograms, saturate on overflow",
    ...
    SECTION("Max per bin = 0") {
        ...
        in.feed_end();
        REQUIRE(out.check(
            accumulated_histogram_event<u16>{42, 43, {0}, 1, 1, true, true}));
        REQUIRE(out.check_end());
    }
    ...
    SECTION("Max per bin = 1") {
        ...
        in.feed(reset_event{46});
        REQUIRE(out.check(
            accumulated_histogram_event<u16>{42, 45, {1}, 2, 1, true, false}));
        in.feed(bin_increment_batch_event<u32>{47, 48, {0}});
        REQUIRE(out.check(histogram_event<u16>{47, 48, {1}, 1, 0}));
        in.feed_end();
        REQUIRE(out.check(
            accumulated_histogram_event<u16>{47, 48, {1}, 1, 0, true, true}));
        REQUIRE(out.check_end());
    }
}
...
TEST_CASE("Accumulate histograms, reset on overflow",
    ...
    SECTION("Max per bin = 1") {
        ...
        in.feed(bin_increment_batch_event<u32>{44, 45, {0}}); // Overflow
        REQUIRE(out.check(
            accumulated_histogram_event<u16>{42, 43, {1}, 1, 0, true, false}));
        REQUIRE(out.check(histogram_event<u16>{44, 45, {1}, 1, 0}));

        SECTION("Normal end") {
            in.feed_end();
            REQUIRE(out.check(accumulated_histogram_event<u16>{
...
TEST_CASE("Accumulate histograms, stop on overflow",
    ...
    SECTION("Max per bin = 1") {
        ...
        in.feed(bin_increment_batch_event<u32>{44, 45, {0}}); // Overflow
        REQUIRE(out.check(
            accumulated_histogram_event<u16>{42, 43, {1}, 1, 0, true, true}));
        REQUIRE(out.check_end());
    }
