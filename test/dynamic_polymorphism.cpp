...
static_assert(
    !handles_event_set_v<virtual_processor<event_set<>>, event_set<e0>>);
static_assert(
    handles_event_set_v<virtual_processor<event_set<e0>>, event_set<e0>>);
static_assert(handles_event_set_v<virtual_processor<event_set<e0, e1>>,
                                  event_set<e0, e1>>);
...
static_assert(
    handles_event_set_v<
        virtual_wrapped_processor<discard_all<event_set<e0>>, event_set<e0>>,
        event_set<e0>>);
static_assert(
    handles_event_set_v<virtual_wrapped_processor<
                            discard_all<event_set<e0, e1>>, event_set<e0, e1>>,
                        event_set<e0, e1>>);
