...
/**
 * \brief Access for `tcspc::late_bound_reader()` acquisition reader.
 *
 * \ingroup context-access
 */
template <typename T> class late_bound_reader_access {
  ...
  public:
    /** \private */
    template <typename F>
    explicit late_bound_reader_access(F set_func_func)
        : set_read_func_func(set_func_func) {}

    /**
     * \brief Set the function implementing a read.
     *
     * \param func the read function, of type `auto(span<T>) ->
     * std::optional<std::size_t>>`.
     */
    template <typename F> void set_read_func(F func) {
        set_read_func_func(func);
    }
};

/**
 * \brief Acquisition reader whose implementation can be set after creation.
 *
 * \ingroup acquisition-readers
 */
template <typename T> class late_bound_reader {
  ...
  public:
    /**
     * \brief Construct with access tracker.
     *
     * \param tracker access tracker for later setting of the read
     * implementation.
     */
    explicit late_bound_reader(
        access_tracker<late_bound_reader_access<T>> &&tracker)
        : trk(std::move(tracker)) {
        trk.register_access_factory([](auto &tracker) {
            auto *self =
                LIBTCSPC_OBJECT_FROM_TRACKER(late_bound_reader, trk, tracker);
            return late_bound_reader_access<T>(
                [self](auto func) { self->set_read_func(func); });
        });
    }

    /** \brief Implements the acquisition reader requirement. */
    auto operator()(span<T> buffer) -> std::optional<std::size_t> {
        return read_func ? read_func(buffer) : std::nullopt;
    }
};
