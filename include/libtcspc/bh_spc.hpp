...
struct bh_spc_event {
    ...
    [[nodiscard]] auto routing_signals() const noexcept -> u8np {
        ...
        return read_u8_at<1>(span(bytes)) >> 4;
    }
    ...
    [[nodiscard]] auto macrotime() const noexcept -> u16np {
        return read_u16le_at<0>(span(bytes)) & 0x0fff_u16np;
    }
    ...
    [[nodiscard]] auto marker_flag() const noexcept -> bool {
        return (read_u8_at<3>(span(bytes)) & (1_u8np << 4)) != 0_u8np;
    }
    ...
    [[nodiscard]] auto gap_flag() const noexcept -> bool {
        return (read_u8_at<3>(span(bytes)) & (1_u8np << 5)) != 0_u8np;
    }
    ...
    [[nodiscard]] auto macrotime_overflow_flag() const noexcept -> bool {
        return (read_u8_at<3>(span(bytes)) & (1_u8np << 6)) != 0_u8np;
    }
     ...
     * count as generated by SPC-160 and SPC-180N.
     *
     * The gap flag is cleared.
     *
     * \param macrotime the marker macrotime; 0 to 4095
...
struct bh_spc600_4096ch_event {
     ...
     */
    [[nodiscard]] auto routing_signals() const noexcept -> u8np {
        return read_u8_at<3>(span(bytes));
    }
    ...
    [[nodiscard]] auto macrotime() const noexcept -> u32np {
        auto const lo8 = u32np(read_u8_at<4>(span(bytes)));
        auto const mid8 = u32np(read_u8_at<5>(span(bytes)));
        auto const hi8 = u32np(read_u8_at<2>(span(bytes)));
     ...
     */
    [[nodiscard]] auto gap_flag() const noexcept -> bool {
        return (read_u8_at<1>(span(bytes)) & (1_u8np << 6)) != 0_u8np;
    }
     ...
     */
    [[nodiscard]] auto macrotime_overflow_flag() const noexcept -> bool {
        return (read_u8_at<1>(span(bytes)) & (1_u8np << 5)) != 0_u8np;
    }
     ...
     */
    [[nodiscard]] auto invalid_flag() const noexcept -> bool {
        return (read_u8_at<1>(span(bytes)) & (1_u8np << 4)) != 0_u8np;
    }
...
struct bh_spc600_256ch_event {
     ...
     */
    [[nodiscard]] auto adc_value() const noexcept -> u16np {
        return u16np(read_u8_at<0>(span(bytes)));
    }
     ...
     */
    [[nodiscard]] auto routing_signals() const noexcept -> u8np {
        return (read_u8_at<3>(span(bytes)) & 0x0f_u8np) >> 1;
    }
     ...
     */
    [[nodiscard]] auto macrotime() const noexcept -> u32np {
        auto const lo8 = u32np(read_u8_at<1>(span(bytes)));
        auto const mid8 = u32np(read_u8_at<2>(span(bytes)));
        auto const hi1 = u32np(read_u8_at<3>(span(bytes))) & 1_u32np;
