 ...
 * macrotime is passed through. In particular, timing events beyond the
 ...
 * (including any that would have had the same macrotime as the \c
 * TriggerEvent).
 *
 * (If a timing event generated by the previous trigger shares the same
 * macrotime as a new trigger, it will only be emitted if some other event
 * (also with the same macrotime) is passed through before the new trigger.
 ...
 * it is expected that the next trigger is not received until a later macrotime
 ...
 * macrotime. However, \c peek must return the correct value (\c std::nullopt)
