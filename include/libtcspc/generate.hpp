 ...
 *
 * If the next \p TriggerEvent is received before the current pattern has been
 * completed, any remaining timing events in the pattern with abstime strictly
 * greater than that of the new \p TriggerEvent are suppressed; pending events
 * whose abstime equals that of the new \p TriggerEvent are emitted before the
 * trigger is passed through.
 *
 ...
 *   trigger and with earlier or equal abstime; discard any remaining
 *   `OutputEvent`s that the previous trigger would have produced; set up new
 *   round of generation; pass through
