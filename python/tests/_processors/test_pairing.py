...
def test_PairOneBetween_event_set_adds_pair():
    node = tcspc.PairOneBetween(0, [1], 1000)
    (out,) = node._map_event_sets([(tcspc.DetectionEvent(),)])
    cpp = [e._cpp_type_name() for e in out]
    assert tcspc.DetectionPairEvent()._cpp_type_name() in cpp


def test_PairOneBetween_codegen():
    code = tcspc.PairOneBetween(0, [1], 1000)._cpp_expression(gencontext, DOWN)
    assert "tcspc::pair_one_between<1, tcspc::default_numeric_traits>(" in code
    assert "DOWN" in code
