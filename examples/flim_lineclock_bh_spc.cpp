...
// TODO: This duplicates much of flim_bruker_bh_spc; it may be better to
// combine the two examples into one program.
...
void usage() {
    print_err(R"(
Usage: flim_lineclock_bh_spc options input_file output_file
...
marker 1 is a valid line clock (start of each scan line). There must not be any
marker 1 events that are not line starts.
...
auto make_processor(settings const &settings,
    ...
    // N.B. 'generate' emits generated events (pixel-stop here) _before_
    // passing through unrelated events (pixel-start here):
    generate<line_start_event, pixel_stop_event>(
        linear_timing_generator(
