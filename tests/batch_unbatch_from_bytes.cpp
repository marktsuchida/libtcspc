...
TEST_CASE("type constraints: batch_from_bytes") {
    ...
    STATIC_CHECK_FALSE(processor<proc_type, std::span<short const>>);
}
...
TEST_CASE("batch_from_bytes") {
    ...
    SECTION("empty input does not emitted batch") {
        in.handle(std::span<std::byte const>());
        in.flush();
    ...
    SECTION("input with whole elements is emitted exactly") {
        ...
        in.handle(std::as_bytes(std::span(data)));
    ...
    SECTION("split elements are emitted in next batch") {
        std::vector<int> data{1, 2, 3};
        in.handle(std::as_bytes(std::span(data)).first(5));
        REQUIRE(out.check(emitted_as::always_rvalue, test_bucket({1})));
        in.handle(std::as_bytes(std::span(data)).subspan(5));
    ...
    SECTION("element can be split over more than 2 inputs") {
        ...
        in.handle(std::as_bytes(std::span(data)).first(1));
        in.handle(std::as_bytes(std::span(data)).subspan(1, 1));
        in.handle(std::as_bytes(std::span(data)).subspan(2, 0));
        in.handle(std::as_bytes(std::span(data)).subspan(2));
    ...
    SECTION("flush throws if buffered bytes remain") {
        ...
        in.handle(std::span(std::as_const(data)));
...
TEST_CASE("unbatch_from_bytes") {
    ...
    SECTION("unaligned input with whole elements emits exactly") {
        std::vector<int> data{1, 2, 3};
        std::array<int, 4> buf{};
        std::memcpy(std::as_writable_bytes(std::span(buf)).subspan(2).data(),
                    std::span(data).data(), sizeof(int) * data.size());
        in.handle(std::as_bytes(std::span(buf))
                      .subspan(2, sizeof(int) * data.size()));
        REQUIRE(out.check(emitted_as::always_lvalue, 1));
        REQUIRE(out.check(emitted_as::always_lvalue, 2));
        REQUIRE(out.check(emitted_as::always_lvalue, 3));
        in.flush();
