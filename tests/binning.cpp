...
TEST_CASE("type constraints: map_to_datapoints") {
    ...
    using proc_type =
        decltype(map_to_datapoints<bulk_counts_event<>, numtraits>(
            count_data_mapper<numtraits>(),
            sink_only<datapoint_event<numtraits>, int>()));
...
TEST_CASE("type constraints: map_to_bins") {
    ...
    using proc_type = decltype(map_to_bins<numtraits>(
        power_of_2_bin_mapper<16, 8, false, numtraits>(),
        sink_only<bin_increment_event<numtraits>, int>()));
...
TEST_CASE("type constraints: cluster_bin_increments") {
    ...
    using proc_type =
        decltype(cluster_bin_increments<start_event, stop_event, numtraits>(
            sink_only<bin_increment_cluster_event<numtraits>, int>()));
    STATIC_CHECK(
...
TEST_CASE("introspect: binning") {
    ...
    check_introspect_simple_processor(
        map_to_datapoints<bulk_counts_event<numtraits>, numtraits>(
            count_data_mapper<numtraits>(), sink_all()));
    check_introspect_simple_processor(
...
TEST_CASE("Map to datapoints") {
    ...
    using out_events = type_list<datapoint_event<numtraits>, misc_event>;
    ...
    REQUIRE(out.check(datapoint_event<numtraits>{42}));
    in.flush();
...
TEST_CASE("Map to bins") {
    ...
    SECTION("Out of range") {
        using out_events =
            type_list<bin_increment_event<numtraits>, misc_event>;
        auto in = feed_input(
        ...
        in.handle(misc_event{42});
        REQUIRE(out.check(emitted_as::same_as_fed, misc_event{42}));
        in.handle(datapoint_event<numtraits>{123});
        in.flush();
        REQUIRE(out.check_flushed());
    }
    ...
    SECTION("Simple mapping") {
        ...
        in.handle(datapoint_event<numtraits>{10});
        REQUIRE(out.check(bin_increment_event<numtraits>{52}));
        in.flush();
        REQUIRE(out.check_flushed());
    }
}
...
TEST_CASE("Power-of-2 bin mapping") {
    ...
    power_of_2_bin_mapper<2, 0, true, numtraits> const m20f;
    ...
    power_of_2_bin_mapper<2, 1, false, numtraits> const m21;
    ...
    power_of_2_bin_mapper<2, 1, true, numtraits> const m21f;
    ...
    power_of_2_bin_mapper<2, 2, false, numtraits> const m22;
    ...
    power_of_2_bin_mapper<2, 2, true, numtraits> const m22f;
    ...
    power_of_2_bin_mapper<12, 8, false, numtraits> const m12_8;
    ...
    power_of_2_bin_mapper<12, 8, true, numtraits> const m12_8f;
...
TEST_CASE("power_of_2_bin_mapper with signed datapoint_type") {
    ...
    power_of_2_bin_mapper<16, 8, false, numtraits> const m;
    CHECK(m(0).value() == 0);
...
TEST_CASE("Linear bin mapping") {
    ...
    linear_bin_mapper<numtraits> const m010{arg::offset{0}, arg::bin_width{1},
                                            arg::max_bin_index<u16>{0},
                                            arg::clamp{clamp}};
    ...
    linear_bin_mapper<numtraits> const m110{arg::offset{1}, arg::bin_width{1},
                                            arg::max_bin_index<u16>{0},
                                            arg::clamp{clamp}};
    ...
    linear_bin_mapper<numtraits> const nn10{arg::offset{-1}, arg::bin_width{1},
                                            arg::max_bin_index<u16>{0},
                                            arg::clamp{clamp}};
    ...
    linear_bin_mapper<numtraits> const m020{arg::offset{0}, arg::bin_width{2},
                                            arg::max_bin_index<u16>{0},
                                            arg::clamp{clamp}};
    ...
    linear_bin_mapper<numtraits> const m120{arg::offset{1}, arg::bin_width{2},
                                            arg::max_bin_index<u16>{0},
                                            arg::clamp{clamp}};
    ...
    linear_bin_mapper<numtraits> const mn20{arg::offset{-1}, arg::bin_width{2},
                                            arg::max_bin_index<u16>{0},
                                            arg::clamp{clamp}};
    ...
    linear_bin_mapper<numtraits> const m0n0{arg::offset{0}, arg::bin_width{-1},
                                            arg::max_bin_index<u16>{0},
                                            arg::clamp{clamp}};
    ...
    linear_bin_mapper<numtraits> const m1n0{arg::offset{1}, arg::bin_width{-1},
                                            arg::max_bin_index<u16>{0},
                                            arg::clamp{clamp}};
    ...
    linear_bin_mapper<numtraits> const mnn0{
    ...
    linear_bin_mapper<numtraits> const m011{arg::offset{0}, arg::bin_width{1},
                                            arg::max_bin_index<u16>{1},
                                            arg::clamp{clamp}};
    ...
    linear_bin_mapper<numtraits> const m111{arg::offset{1}, arg::bin_width{1},
                                            arg::max_bin_index<u16>{1},
                                            arg::clamp{clamp}};
    ...
    linear_bin_mapper<numtraits> const mn11{arg::offset{-1}, arg::bin_width{1},
                                            arg::max_bin_index<u16>{1},
                                            arg::clamp{clamp}};
    ...
    linear_bin_mapper<numtraits> const m0n1{arg::offset{0}, arg::bin_width{-1},
                                            arg::max_bin_index<u16>{1},
                                            arg::clamp{clamp}};
    ...
    linear_bin_mapper<numtraits> const m1n1{arg::offset{1}, arg::bin_width{-1},
                                            arg::max_bin_index<u16>{1},
                                            arg::clamp{clamp}};
    ...
    linear_bin_mapper<numtraits> const mnn1{
    ...
    linear_bin_mapper<numtraits> const maxint{
    ...
    struct numtraits_u32 : numtraits {
        using datapoint_type = u32;
    };
    linear_bin_mapper<numtraits_u32> const maxuint{
        arg::offset{0u}, arg::bin_width{65536u},
        arg::max_bin_index<u16>{65535}, arg::clamp{clamp}};
    ...
    linear_bin_mapper<numtraits> const flipped{
...
TEST_CASE("unique_bin_mapper") {
    using datapoint_type = default_numeric_traits::datapoint_type;
...
TEST_CASE("Batch bin increments") {
    ...
    using out_events =
        type_list<bin_increment_cluster_event<numtraits>, misc_event>;
    auto const valcat = GENERATE(feed_as::const_lvalue, feed_as::rvalue);
    auto ctx = context::create();
    auto in = feed_input(
    ...
    SECTION("Start with no stop ignored") {
        ...
        in.handle(bin_increment_event<numtraits>{123});
        in.flush();
        REQUIRE(out.check_flushed());
    }
    ...
    SECTION("Events passed only between start and stop") {
        ...
        REQUIRE(out.check(
            bin_increment_cluster_event<numtraits>{test_bucket<u32>({123})}));
        in.handle(start_event{45});
        in.handle(bin_increment_event<numtraits>{124});
        in.handle(bin_increment_event<numtraits>{125});
        in.handle(stop_event{48});
        REQUIRE(out.check(bin_increment_cluster_event<numtraits>{
