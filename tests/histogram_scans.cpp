...
TEMPLATE_TEST_CASE_SIG("type constraints: histogram_scans", "", ((hp P), P),
    ...
    SECTION("no concluding event") {
        using proc_type = decltype(histogram_scans<P, reset_event>(
            arg::num_elements<std::size_t>{256},
...
TEST_CASE("introspect: histogram_scans") {
    check_introspect_simple_processor(histogram_scans(
        arg::num_elements<std::size_t>{1}, arg::num_bins<std::size_t>{1},
...
TEMPLATE_TEST_CASE_SIG(
    ...
    auto in = feed_input(
        valcat,
        histogram_scans<P | hp::reset_after_scan, reset_event>(
            arg::num_elements<std::size_t>{2}, arg::num_bins<std::size_t>{2},
            arg::max_per_bin<u16>{65535}, bsource,
            capture_output<all_output_events>(
                ctx->tracker<capture_output_access>("out"))));
    in.require_output_checked(ctx, "out");
...
TEMPLATE_TEST_CASE_SIG(
    ...
    auto in = feed_input(
        valcat,
        histogram_scans<P | hp::clear_every_scan, reset_event>(
            arg::num_elements<std::size_t>{2}, arg::num_bins<std::size_t>{2},
            arg::max_per_bin<u16>{65535}, bsource,
            capture_output<all_output_events>(
                ctx->tracker<capture_output_access>("out"))));
...
TEMPLATE_TEST_CASE_SIG(
    ...
    auto in = feed_input(
        valcat,
        histogram_scans<P | hp::no_clear_new_bucket, reset_event>(
            arg::num_elements<std::size_t>{2}, arg::num_bins<std::size_t>{2},
            arg::max_per_bin<u16>{65535}, bsource,
            capture_output<all_output_events>(
                ctx->tracker<capture_output_access>("out"))));
...
TEST_CASE("histogram_scans saturate_on_overflow") {
    ...
    auto in = feed_input(
        valcat,
        histogram_scans<hp::saturate_on_overflow, reset_event>(
            arg::num_elements<std::size_t>{2}, arg::num_bins<std::size_t>{2},
            arg::max_per_bin<u16>{3}, bsource,
            capture_output<all_output_events>(
                ctx->tracker<capture_output_access>("out"))));
