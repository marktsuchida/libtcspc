...
TEST_CASE("Read u8 compiles") {
    ...
    CHECK(read_u8_at<0>(std::span(data)) == 0_u8np);
    CHECK(read_u8_at<1>(std::span(std::as_const(data))) == 0_u8np);
}

TEST_CASE("Read u16 compiles") {
    std::array<std::byte, 3> data{};
    CHECK(read_u16le_at<0>(std::span(data)) == 0_u16np);
    CHECK(read_u16le_at<1>(std::span(std::as_const(data))) == 0_u16np);
}
...
TEST_CASE("Read u16np impl") {
    ...
    SECTION("Zero") {
        data = {0, 0};
        REQUIRE(f(std::as_bytes(std::span(data))) == 0_u16np);
    }

    SECTION("Low byte") {
        auto x = GENERATE(std::uint8_t(0x01), 0x7f, 0x80, 0xff);
        data = {x, 0};
        REQUIRE(f(std::as_bytes(std::span(data))) == u16np(x));
    }

    SECTION("High byte") {
        auto x = GENERATE(std::uint8_t(0x01), 0x7f, 0x80, 0xff);
        data = {0, x};
        REQUIRE(f(std::as_bytes(std::span(data))) == u16np(x * 0x100u));
    }
}
...
TEST_CASE("Read u32np impl") {
    ...
    SECTION("Zero") { REQUIRE(f(std::as_bytes(std::span(data))) == 0_u32np); }

    SECTION("Single byte") {
        auto x = GENERATE(std::uint8_t(0x01), 0x7f, 0x80, 0xff);
        // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-constant-array-index)
        data[byte] = x;
        REQUIRE(f(std::as_bytes(std::span(data))) ==
                u32np(x * (1u << (8 * byte))));
    }
    ...
    SECTION("Sanity") {
        ...
        data[2] = 3;
        data[3] = 4;
        REQUIRE(f(std::as_bytes(std::span(data))) == 0x0403'0201_u32np);
    }
}
...
TEST_CASE("Read u64np impl") {
    ...
    SECTION("Zero") { REQUIRE(f(std::as_bytes(std::span(data))) == 0_u64np); }
