...
TEST_CASE("convert_with_check") {
    ...
    CHECK_THROWS_AS(convert_with_check<i8>(128), std::range_error);
    CHECK_THROWS_AS(convert_with_check<i8>(-129), std::range_error);
    STATIC_CHECK(convert_with_check<u8>(255) == u8(255));
    STATIC_CHECK(convert_with_check<u8>(0) == u8(0));
    CHECK_THROWS_AS(convert_with_check<u8>(256), std::range_error);
    CHECK_THROWS_AS(convert_with_check<u8>(-1), std::range_error);
}

TEST_CASE("add_with_check") {
    CHECK(add_with_check(u8(254), u8(1)) == u8(255));
    CHECK(add_with_check(i8(126), i8(1)) == i8(127));
    CHECK_THROWS_AS(add_with_check(u8(254), u8(2)), std::overflow_error);
    CHECK_THROWS_AS(add_with_check(i8(126), i8(2)), std::overflow_error);
    CHECK(add_with_check(i8(-127), i8(-1)) == i8(-128));
    CHECK_THROWS_AS(add_with_check(i8(-127), i8(-2)), std::overflow_error);
}

TEST_CASE("subtract_with_check") {
    CHECK(subtract_with_check(u8(1), u8(1)) == u8(0));
    CHECK(subtract_with_check(i8(-127), i8(1)) == i8(-128));
    CHECK_THROWS_AS(subtract_with_check(u8(1), u8(2)), std::overflow_error);
    CHECK_THROWS_AS(subtract_with_check(i8(-127), i8(2)), std::overflow_error);
    CHECK(subtract_with_check(i8(126), i8(-1)) == i8(127));
    CHECK_THROWS_AS(subtract_with_check(i8(126), i8(-2)), std::overflow_error);
}
